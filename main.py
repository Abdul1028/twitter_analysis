...
st.sidebar.header("Project:[Owner](https://github.com/Abdul1028)")
...
if username:
    ...
    api = tweepy.API(auth)
    tweets = api.user_timeline(screen_name= username , count=No_of_tweets, include_rts=False, tweet_mode='extended')
    for tweet in tweets:
        ...
        Total_tweets.append(refined_tweet)

    # for a, tweet in enumerate(
...
def data_frame(t_data):
    return pd.DataFrame(t_data, columns=[ 'user', 'text', 'favorite_count',
...
client = pymongo.MongoClient(
    "mongodb+srv://pnrajk:Abdul1028@cluster0.rlu7bvd.mongodb.net/?retryWrites=true&w=majority")
db = client.twitterscraping
col = db.scraped_tweets
scr_data = {"Scraped_word": username,
...
if df.empty:
    ...
    st.success(f"**:green[{username} tweets]:thumbsup:**")
    st.write(df)
